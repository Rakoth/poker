...
class GameTest < ActiveSupport::TestCase
  ...
  test "method next_level should do nothing if game in wait status" do
    game = games(:wait)
    blind_level = game.blind_level
    game.next_level
    assert_equal blind_level, game.blind_level
  end
  ...
  test "method next_level should change blind level and next level time if game in start status and its time" do
    ...
    game.type = type
    game.next_level
    assert_equal blind_level + 1, game.blind_level, "blind_level should be increase by 1"
  ...
  test "method next_level should set next_level_time to nil if blind_level is max" do
    ...
    game.next_level_time = Time.now
    game.next_level
    assert_equal blind_level, game.blind_level, "blind level must be same"
